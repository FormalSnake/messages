...
import { chatTitle, conversationChats, searchChats, type Chat } from '@messages/core'
...
export function Switcher({ onClose }: { onClose: () => void }) {
  ...
  const results = useMemo(() => searchChats(conversationChats(state), query, RESULT_LIMIT), [state.chats, state.primaryOf, query])
